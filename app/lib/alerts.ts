 ...
 *
 * Each call always creates a fresh alert (unlike createMeetingUpdateAlert):
 * a no-agenda placeholder call today and a real agenda-backed call once the
 * scraper finds one are deliberately distinct alerts, not the same content
 * repeated (see upcoming-alert/route.ts's noagenda-sentinel comment). But
 * before creating the new one, any earlier non-terminal (DRAFTED or
 * SENT_TO_ADMINS) MEETING_UPCOMING alert for this meeting is canceled —
 * otherwise a no-agenda placeholder that already reached SENT_TO_ADMINS
 * would sit there permanently once superseded by the real alert, since
 * nothing else ever resolves it (scheduledFor stays null for this alert
 * type, so the scheduled drain never touches it either).
 */
export async function createMeetingUpcomingAlert(
  ...
  await prisma.alert.updateMany({
    where: {
      ...
      status: { in: ["DRAFTED", "SENT_TO_ADMINS"] },
    },
    data: {
      status: "CANCELED",
      canceledAt: new Date(),
      canceledBy: "system:superseded-by-new-upcoming-alert",
    },
  });
