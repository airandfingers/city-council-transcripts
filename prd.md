...
- ✅ FEAT-SEARCH-SERVERSIDE-SURFACE-001 — Expand search to summary items/topics, move server-side (deliberately no Postgres FTS/`pg_trgm` — a server-side function reusing existing search.ts logic, at this data volume; explicit revisit trigger documented)
...
**AC-3's migration decision, resolved before building anything:** brought to the user rather than picked unilaterally, since it changes real architecture. Checked the actual data first: 381 meetings, ~9K `MeetingSummaryItem`/`TopicSummary` rows total (2026-09-09) — tiny. Both original candidates (Postgres FTS, `pg_trgm`) would have introduced matching semantics different from what's already shipped: FTS's stemming/stopword removal, `pg_trgm`'s fuzzy/similarity matching (which `FEAT-SEARCH-NORMALIZE-HIGHLIGHT-001`'s own AC-4 explicitly ruled out — "no fuzzy/trigram matching... exact substring per normalized token"). **Chose instead: no new extension, no migration.** A server-side function (`searchMeetingsForCity` in `cityData.ts`) reuses `app/lib/search.ts`'s existing normalize/match/snippet functions verbatim against a broader Prisma-fetched surface, called via a Server Action (`app/actions/searchMeetings.ts`) — zero semantic drift by construction, since it's the same code, not a reimplementation. Documented as a deliberate scale tradeoff, with an explicit revisit trigger (a second city at Seattle's row-count, or profiled latency), rather than building FTS/`pg_trgm` infra ahead of any real need for it.

**AC-4's Suspense fallback got real content, not a spinner.** `app/[state]/[city]` is cached indefinitely (`revalidate = false`), so whatever the Suspense fallback renders is literally what ships in that cached HTML until a visitor's browser hydrates `MeetingFilter` — not a brief build-time-only placeholder. Rather than a loading skeleton, the fallback (`MeetingListFallback`) server-renders the same meetings, newest-first, with no search/sort/status controls — the page's core content stays present and functional without JS, and static rendering (`next build`'s `●` marker) is unaffected — verified in the build output.

**Verified against real data — the real function, and the real browser round trip, not just a mirror and a type-check.** First ran `searchMeetingsForCity` itself directly (via `tsx`) against production Neon data: `"data centers"` against Fort Collins surfaces a second meeting (`2026-02-10/city-council-work-session`) the old title/summary/logline-only search missed entirely — a real instance of exactly the gap this story exists to close. Then exercised the actual browser path end-to-end (Playwright against a real `next dev` + Neon): typing a query updates the URL via the server action and renders the "Matches in Key Decisions: …" snippet live; loading `?q=housing` directly seeds the input from the URL; typing over a real (non-`replace`) prior navigation and pressing Back correctly reverts both the URL *and* the input/results together, not just the address bar — a real desync risk in the debounce/URL-sync effects, fixed (`lastSyncedQRef`) and confirmed fixed, not just reasoned about.

**Known limitation, found via the same real-data check, not fixed — inherited, not introduced.** That `2026-02-10` match is a genuine instance of `FEAT-SEARCH-NORMALIZE-HIGHLIGHT-001`'s own accepted tradeoff (AND-per-token, "any order, anywhere," not phrase matching) surfacing more visibly now that there's more searchable text to scatter across: "data" only appears in that meeting's summary ("data transparency," an unrelated public-comment concern) and "centers" only appears in one unrelated action item ("neighborhood centers," an economic-development wording tweak) — not "data centers" as a phrase or topic anywhere in the meeting. Checked whether scoping the AND-check to one field-pair at a time (visible haystack + one extra fragment, never two unrelated extra fragments together) would fix this — it wouldn't: the visible haystack alone already contributes one of the two tokens, so even the narrowest reasonable scoping still combines them. This is the same tradeoff already accepted for `summary`'s own free-form prose, just more likely to fire once decisions/actions/topics multiply the number of independent fragments a query's tokens can scatter across. Not fixed here — phrase-adjacency or proximity scoring would be a real design change to an already-shipped, deliberate choice, not a bug in this story's code. Flagged for the user's call if it becomes a real nuisance in practice.

**Acceptance Criteria:**
- [x] AC-1: Searchable surface includes `MeetingSummaryItem` text (key decisions, action items, timeline bullets) and `TopicSummary` titles/key points.
- [x] AC-2: Matching moves server-side (`searchMeetingsForCity`, called via `app/actions/searchMeetings.ts`) — closes AC-6.2 above and the in-code note at `MeetingFilter.tsx:17-22`. The broader surface (key decisions/action items/timeline/topics) is fetched and matched entirely server-side and never reaches the client payload; the pre-existing `summary`/`logline` fields are unchanged (still fetched for card display regardless of search, as before — not new cost).
- [x] AC-3: Server-side matching reproduces the exact token-AND/punctuation-folding semantics from `FEAT-SEARCH-NORMALIZE-HIGHLIGHT-001` by literally reusing its functions — no new extension or migration; see the design note above for why.
- [x] AC-4: Filter state (the `q` query param) syncs to the URL, debounced, via `router.replace` — closes AC-6.3 above. Suspense boundary added around `MeetingFilter` in `app/[state]/[city]/page.tsx`; confirmed the route still statically renders (`next build`'s `●` marker unchanged).
- [x] AC-5: Match-context snippets extend to the newly searchable fields ("Matches in Key Decisions: …", "Matches in Topic: …"), rendered the same way as the pre-existing "Matches in summary: …" line.

**Files Modified:** `app/lib/cityData.ts` (`searchMeetingsForCity`, `MeetingSearchResult`), `app/actions/searchMeetings.ts` (new), `app/components/MeetingFilter.tsx` (debounced server search + URL sync), `app/components/MeetingCard.tsx` (`extraSnippet` prop), `app/[state]/[city]/page.tsx` (Suspense boundary + `MeetingListFallback`).
